...
from qutip import Qobj, sigmax, sigmay, sigmaz, identity, tensor, QobjEvo
...
class TestPulse:
    ...
    def test_noisy_pulse(self):
        ...
        tlist_noise = np.array([0.0, 1.0, 2.5, 3.0])
        coeff_noise = np.array([0.0, 0.5, 0.1, 0.5])
        ...
        noise_qu, c_ops = pulse1.get_noisy_qobjevo(2)
        assert_allclose(
            pulse1.get_full_tlist(), np.array([0.0, 0.5, 1.0, 2.0, 2.5, 3.0])
        )
        ...
        expected = QobjEvo(
            [
                ...
                [
                    ...
                    np.array([0.0, 0.0, 0.5, 0.5, 0.1, 0.5]),
                ],
            ],
            tlist=np.array([0.0, 0.5, 1.0, 2.0, 2.5, 3.0]),
            order=0,
        )
            ...
            if isconstant:
                ...
                expected = QobjEvo(
                    [
                        ...
                        np.array([0.0, 0.1, 0.1, 0.2, 0.2, 0.3]),
                    ],
                    tlist=np.array([0.0, 0.5, 1.0, 2.0, 2.5, 3.0]),
                    order=0,
                )
    ...
    def test_pulse_constructor(self):
        ...
        assert_allclose(pulse3.get_ideal_qobjevo(2)(0).norm(), 0.0)

        pulse4 = Pulse(None, None)  # Dummy empty ham
        assert_allclose(pulse4.get_ideal_qobjevo(2)(0).norm(), 0.0)

        tlist_noise = np.array([1.0, 2.5, 3.0])
