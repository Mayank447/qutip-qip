...
class TestCircuitProcessor:
    def test_control_and_coeffs(self):
        ...
        coeffs = np.array([[1.0, 2.0, 3.0], [3.0, 2.0, 1.0]])
    ...
    def testSpline(self):
        ...
        assert_allclose(
            ideal_qobjevo(0.5).data.to_array(), sigmaz().data.to_array() * 0.0
        )

        # Verify noise inherits the step property
        noisy_qobjevo, c_ops = processor.get_qobjevo(noisy=True)
        assert_allclose(
            noisy_qobjevo(0.5).data.to_array(), sigmaz().data.to_array() * 0.0
        )
    ...
    def testGetObjevo(self):
        ...
        assert_equal(
            unitary_qobjevo.to_list()[0][0],
            processor.get_qobjevo(noisy=False)[0].to_list()[0][0],
        ...
        assert_allclose(actual_val, expected_coeff_val, rtol=1.0e-5)
    ...
    def testNoise(self):
        ...
        tlist = np.array([0.0, np.pi / 2.0])
        a = destroy(2)
        proc = Processor(num_qubits=2)
        proc.add_control(sigmax(), targets=1, label="sx")
        proc.set_all_coeffs({"sx": np.array([1.0])})
        proc.set_all_tlist(tlist)
    ...
    def testDrift(self):
        ...
        tlist = np.array([0.0, np.pi, 2 * np.pi, 3 * np.pi])
    ...
    def test_pulse_mode(self):
        ...
        processor.set_coeffs({"sx": np.array([1.0, 2.0, 3.0])})
        processor.set_tlist({"sx": np.array([0.0, 1.0, 2.0, 3.0])})
    ...
    def test_max_step_size(self):
        ...
        final_state = processor.run_state(
            init_state, options={"max_step": 10000}  # too large max_step
        ).states[-1]
